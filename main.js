     ...
     if (xhr.status === 200) {
       ...
       images.reverse().forEach((image) => {
         ...
         const lightOnButton = document.createElement("button");
           ...
           lightOnButton.addEventListener("click", () => fetchTextureById(image._id, "light-on"));

           // 创建关灯按钮
           const lightOffButton = document.createElement("button");
           lightOffButton.textContent = "關燈測試";
           lightOffButton.addEventListener("click", () =>  fetchTextureById(image._id, "light-off"));
